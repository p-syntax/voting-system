...
const VoterLogin = () => {
  ...
  const stopCamera = () => {
    ...
    setCameraActive(false); // ✅ this hides the video
  };
  ...
  const captureFace = async () => {
    ...
    try {
      ...
      stopCamera(); // ✅ stops camera AND hides video
    } catch (err) {
  ...
  const handleLogin = async () => {
    ...
    try {
      if (!faceDescriptor) {
        throw new Error("Please capture your image first.");
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-6">
        ...
        <input
          className="w-full border-2 border-black rounded-md px-3 py-2 mb-4"
        ...
        {/* Camera Preview (hidden when cameraActive = false) */}
        <div className="mt-4">
            <video
                ref={videoRef}
                muted
